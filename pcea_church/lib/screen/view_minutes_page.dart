...
class ViewMinutesPage extends StatefulWidget {
  const ViewMinutesPage({super.key});

  @override
  State<ViewMinutesPage> createState() => _ViewMinutesPageState();
}

class _ViewMinutesPageState extends State<ViewMinutesPage> {
  ...
  Future<void> _fetchMinutes() async {
    ...
    try {
      ...
      final endpoints = ['/minutes/mine', '/minutes', '/minutes/all'];

      for (final endpoint in endpoints) {
        print('Trying endpoint: ${Config.baseUrl}$endpoint');
        ...
        if (res.statusCode == 200) {
          ...
          if (body is Map) {
            ...
            for (final key in possibleKeys) {
              if (body.containsKey(key)) {
                ...
                if (minutesData is List) {
                  print(
                    'Key "$key" contains a List with ${minutesData.length} items',
                  );
            ...
            if (data.isEmpty && body.isNotEmpty) {
              print(
                'No array found, treating entire response as single minute',
              );
          ...
          print('Endpoint $endpoint failed with status ${res.statusCode}');
        }
      }

      // If all endpoints failed
      setState(() {
        _error = 'No working endpoint found. Tried: ${endpoints.join(', ')}';
        _loading = false;
      });
    } catch (e) {
  ...
  List<Map<String, dynamic>> get _filteredMinutes {
    ...
    return _minutes.where((m) {
      final title = (m['title'] ?? m['agendaTitle'] ?? '')
          .toString()
          .toLowerCase();
      final type = (m['meetingType'] ?? m['type'] ?? '')
          .toString()
          .toLowerCase();
      final num = (m['minuteNumber'] ?? m['number'] ?? '')
          .toString()
          .toLowerCase();
  ...
  void _showDetails(Map<String, dynamic> item) {
    showModalBottomSheet(
      ...
      builder: (ctx) {
        ...
        final agendaDetails = Map<String, dynamic>.from(
          item['agendaDetails'] ?? {},
        );
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      _Chip(
                        ...
                        label:
                            (item['meetingType'] ?? item['type'] ?? 'General')
                                .toString(),
                      ),
                      _Chip(
                        icon: Icons.confirmation_number,
                        label: (item['minuteNumber'] ?? item['number'] ?? '')
                            .toString(),
                      ),
                      _Chip(
                        icon: Icons.event,
                  ...
                  else if (agendas is List && agendas.isNotEmpty)
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Text(
                        ...
                        const SizedBox(height: 8),
                        ...List<Widget>.generate(agendas.length, (i) {
                          final a = agendas[i].toString();
                          return _AgendaTile(
                            title: '${i + 1}. $a',
                            content: (agendaDetails[a] ?? '').toString(),
                          );
                        }),
                      ],
                    ),
          ...
          Padding(
            ...
            child: TextField(
              decoration: InputDecoration(
                ...
                prefixIcon: const Icon(Icons.search, color: Color(0xFF35C2C1)),
                filled: true,
                      ...
                      itemBuilder: (ctx, i) {
                        ...
                        final title =
                            (m['title'] ??
                                ...
                                .toString();
                        final type =
                            (m['meetingType'] ?? m['type'] ?? 'General')
                                .toString();
                        final number = (m['minuteNumber'] ?? m['number'] ?? '')
                            .toString();
...
class _Chip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Chip(
      avatar: Icon(icon, size: 16, color: const Color(0xFF35C2C1)),
      label: Text(label),
